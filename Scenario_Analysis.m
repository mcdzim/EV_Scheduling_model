...
Scenario{1,10} = [10000, 0.3, 0.7, 40, 7];
...
% Scenario{1,1} = [10000, 0.5, 0.9, 40, 3];
% Scenario{1,2} = [10000, 0.4, 0.9, 40, 3];
% Scenario{1,3} = [10000, 0.3, 0.7, 40, 7];

ASAP_results(1:24, 1) = hour;
ALAP_results(1:24, 1) = hour;
MidP_results(1:24, 1) = hour;
...
%Run Simulation
simulation = MachineModel(Scenario_current);
...
ASAP_results(1:24, x+1) = simulation(1:24, 1);
ALAP_results(1:24, x+1) = simulation(1:24, 2);
MidP_results(1:24, x+1) = simulation(1:24, 3);
...
axis([0 24 -0 1])
print('Simulation' + string(x) ,'-dpng')
